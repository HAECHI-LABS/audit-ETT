...
contract Operatable {
  ...
  modifier onlyOperator() {
    require(isOperator(msg.sender), "OperatorRole: caller does not have the Locker role");
    _;
  }
...
contract Lockable {
  using Roles for Roles.Role;
  /**
    * @dev Emitted when the lock is triggered by a locker (`account`).
    */
  event Locked(address account);

  /**
    * @dev Emitted when the lock is lifted by a locker (`account`).
    */
  event Unlocked(address account);

  Roles.Role private _locks;

  /**
    * @dev Return whether account is locked or not.
    */
  function isLocked(address account) public view returns (bool) {
    return _locks.has(account);
  }

  /**
    * @dev Called by a locker to lock.
    */
  ...
  /**
    * @dev Called by a locker to unlock.
    */
  function _unlock(address account) internal {
    _locks.remove(account);
 ...
 * @dev KIP7 modified with lockable transfers.
 */
contract KIP7Lockable is KIP7Pausable, Lockable {

  function transfer(address to, uint256 value) public returns (bool) {
    require(!isLocked(to), "Locked");
  ...
  function approve(address spender, uint256 value) public returns (bool) {
    require(!isLocked(spender), "Locked");
    require(!isLocked(msg.sender), "Locked");
    return super.approve(spender, value);
  }
}
...
contract ETT is KIP7Lockable, KIP7Metadata, Ownable, Operatable {

  uint private _initialSupply;

  constructor() KIP7Metadata("egg coin", "ETT", 18) public {
    _initialSupply = 9000000e18;
  ...
  /**
    * @dev Called by an owner to add operator.
    */
  ...
  /**
  * @dev Called by an owner to pause entire network.
  */
  ...
  /**
  * @dev Called by an owner to unpause entire network.
  */
  ...
  /**
  * @dev Called by an operator to lock account.
  */
  function lock(address account) public onlyOperator {
    _lock(account);
  }

  /**
    * @dev Called by an operator to unlock account.
    */
  function unlock(address account) public onlyOperator  {
    _unlock(account);
  }

  /**
    * @dev Called by an operator to burn self account
    */
